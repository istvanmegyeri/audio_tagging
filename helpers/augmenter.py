...
def combine(x1, x2, r1, r2, eps=1e-4):
    return np.multiply(r1, np.multiply(1.0 / np.maximum(np.max(np.abs(x1)), eps), x1)) + \
           np.multiply(r2, np.multiply(1.0 / np.maximum(np.max(np.abs(x2)), eps), x2))
